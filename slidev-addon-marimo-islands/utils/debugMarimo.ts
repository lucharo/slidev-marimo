...
export function debugMarimo(): DebugInfo {
  ...
  info.resourcesLoaded.script = !!document.querySelector('#marimo-islands-script')
  if (!info.resourcesLoaded.script) {
    info.recommendations.push('❌ Marimo islands script not loaded. Check setup/main.ts')
  }
  ...
  // Check Google Fonts
  info.resourcesLoaded.fonts = !!document.querySelector('link[href*="fonts.googleapis.com"]')
  if (!info.resourcesLoaded.fonts) {
    info.recommendations.push('⚠️  Google Fonts not loaded (may cause rendering issues)')
  }
  ...
  info.resourcesLoaded.marimoFilename = !!document.querySelector('marimo-filename')
  ...
  if (!info.customElement.registered) {
    info.recommendations.push('❌ marimo-island custom element NOT registered. Script may not have loaded/executed.')
  } else {
    info.recommendations.push('✓ marimo-island custom element registered')
  }

  // Count island elements in DOM
  info.customElement.elementCount = document.querySelectorAll('marimo-island').length
  if (info.customElement.elementCount === 0) {
    info.recommendations.push('⚠️  No <marimo-island> elements found in DOM')
  } else {
    info.recommendations.push(`✓ Found ${info.customElement.elementCount} island element(s)`)
  }
...
export function checkPyodide() {
  ...
  if (win.pyodide) {
    ...
    console.log('⚠️  Pyodide not yet initialized (this is normal until first island runs)')
  }
